//
//  SegamentSliderViewMenuAutoView.m
//  QDCommentProject
//
...
#import "SegamentSliderViewMenuAutoView.h"
#import "UUPanGestureScrollView.h"
#import "UILabel+Extention.h"


CGFloat const  TitleScroviewAlpan =  0.7f;  //透明度
CGFloat const  tagHeight =  44; // 标签栏高度
CGFloat const  BtnFont =  17; //滑块字体大小

@interface SegamentSliderViewMenuAutoView ()<UIScrollViewDelegate>
{
...
@property (nonatomic,weak)UUPanGestureScrollView *titleScrollview;

@property (nonatomic,weak)UUPanGestureScrollView *CtrlScroollview;
...
- (void)addChildViewControllers {
    UUPanGestureScrollView *ctrlScrollview = [[UUPanGestureScrollView alloc] init];
...
-(void)viewDidLayoutSubviews
{
    ...
    /** 容器布局   */
    self.CtrlScroollview.frame = CGRectMake(0,0,kScreenWidth, kScreenHeight - tagHeight) ;
    self.CtrlScroollview.contentSize = CGSizeMake(self.CtrlArr.count * kScreenWidth , kScreenHeight - tagHeight);
    
    /**   下划线  */
    _toolsView.frame = CGRectMake(0, tagHeight - 0.5, kScreenWidth, 0.5);
    
    /**  标签容器布局   */
    self.titleScrollview.frame = CGRectMake(0, 0, kScreenWidth, tagHeight );
    ...
    for (UIView *titleScrollview in self.titleScrollview.subviews) {
        if ([[titleScrollview class] isEqual:[UIButton class]]) {
            ...
            //每个btn宽度
          CGFloat f =  [UILabel getSinglelineStringWidth:[self.childViewControllers[i] title] andFont:[UIFont systemFontOfSize:BtnFont]] +10;
            Zwidth +=f;  //总宽度
    ...
    if (Zwidth<kScreenWidth) {
        btnwidth =kScreenWidth/self.childViewControllers.count;
    }
    [self.titleScrollview addSubview:self.backView];
    if (Zwidth<kScreenWidth) {
        //titlteBtn 居中显示
        self.titleScrollview.contentSize =CGSizeMake((kScreenWidth-Zwidth) / 2 + Zwidth,tagHeight);
    }else{
    ...
    if (self.ScorllviewIndex != 0) {
        ...
        offset.x = self.ScorllviewIndex * kScreenWidth;
...
-(void)NewCreateScorllview{
    // UUPanGestureScrollView 避免和滑动手势冲突
    UUPanGestureScrollView *titleScorllview  = [[UUPanGestureScrollView alloc] init];
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView {
    ...
    //这里是默认进入那个控制器
    NSLog(@"x_ScorllviewIndex = %lu" ,(long)_ScorllviewIndex) ;
    // 一些临时变量
    ...
    NSLog(@"---%@--",NSStringFromClass(button.class));
    //滑块位置
    ...
    NSLog(@"button.center.x = %lf ,width/2 = %lf " ,button.center.x , width/2) ;
